...
export function ComparisonSlider({ before, after, label }: { before: string; after: string; label: string }) {
  ...
  return (
    <div ref={frame} className="relative aspect-[4/3] overflow-hidden rounded-lg bg-[var(--surface-2)] [--reveal:52%]">
      ...
      <div className="pointer-events-none absolute inset-y-0 left-[var(--reveal)] w-px -translate-x-1/2 bg-white shadow-[0_0_0_1px_rgba(0,0,0,.15)]">
        <span className="absolute left-1/2 top-1/2 grid size-11 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border border-black/10 bg-white text-[#15201b] shadow-[0_8px_24px_rgba(0,0,0,.2)]"><ArrowsLeftRight size={19} weight="bold" /></span>
      </div>
