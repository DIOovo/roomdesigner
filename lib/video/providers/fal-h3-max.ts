...
export class FalH3MaxVideoProvider implements VideoGenerationProvider {
  ...
  async generateVideo(input: GenerateVideoInput): Promise<GenerateVideoResult> {
    const result = await fal.subscribe(this.model, { input: this.mapInput(input), startTimeout: 30 });
    return this.mapResult(result.data as FalH3Output, result.requestId);
  }

  async submitVideo(input: GenerateVideoInput) {
    const result = await fal.queue.submit(this.model, { input: this.mapInput(input), startTimeout: 30 });
  ...
  async getVideoStatus(providerTaskId: string): Promise<VideoQueueStatus> {
    const status = await fal.queue.status(this.model, { requestId: providerTaskId, logs: false, abortSignal: AbortSignal.timeout(20_000) });
    ...
    const result = await fal.queue.result(this.model, { requestId: providerTaskId, abortSignal: AbortSignal.timeout(20_000) });
  ...
  private mapInput(input: GenerateVideoInput) {
    if (input.seconds !== 5) throw new Error("fal H3 Max requires a 5-second duration in the Roomorphic real pipeline.");
    return {
