...
export function calculateEntitlements(input: {
  ...
  if (!input.authenticated) {
    ...
    return baseEntitlements(false, remaining, 0, 0, "free", null, null);
  }
  ...
  const visibleSubscription = active ?? [...subscriptions].sort((a, b) => periodEnd(b) - periodEnd(a))[0];
  ...
  return baseEntitlements(
    ...
    creditPack,
    plan,
    visibleSubscription?.status ?? null,
    visibleSubscription?.current_period_end ?? null,
  );
}
...
function periodEnd(subscription: SubscriptionState) {
