...
import { createInputUpload, signUploadedInput, type InputImageKind } from "@/lib/assets/frame-assets";
...
export async function POST(request: Request) {
  let kind: InputImageKind = "room";
  try {
    const body = await readJson(request);
    const parsedKind = parseInputImageKind(body?.kind);
    if (!parsedKind) return NextResponse.json({ error: "Choose a valid image upload type." }, { status: 400 });
    kind = parsedKind;
    ...
    const upload = await createInputUpload({ admin, contentType: body!.contentType as "image/jpeg" | "image/png", ownerId: owner.ownerId!, kind });
    ...
    console.error("Input image upload preparation failed", { kind, reason: error instanceof Error ? error.name : "unknown" });
    return NextResponse.json({ error: kind === "reference" ? "The reference image upload could not be prepared. Please try again." : "The room photo upload could not be prepared. Please try again." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const admin = getSupabaseAdmin();
    if (!admin) return NextResponse.json({ error: "Photo uploads are temporarily unavailable." }, { status: 503 });
    const imageUrl = await signUploadedInput({ admin, path: body.path, ownerId: owner.ownerId!, kind });
    return NextResponse.json({ imageUrl });
  } catch (error) {
    console.error("Input image upload completion failed", { kind, reason: error instanceof Error ? error.name : "unknown" });
    return NextResponse.json({ error: safeUploadCompletionError(error, kind) }, { status: 400 });
  }
}
...
async function readJson(request: Request) {
  ...
  try {
    return await request.json() as { contentType?: unknown; size?: unknown; path?: unknown; kind?: unknown };
  } catch {
    return null;
  }
}

function parseInputImageKind(value: unknown): InputImageKind | null {
  if (value === undefined || value === "room") return "room";
  return value === "reference" ? "reference" : null;
}

function safeUploadCompletionError(error: unknown, kind: InputImageKind) {
  const message = error instanceof Error ? error.message : "";
  if (kind === "reference" && (message === "The room photo could not be processed safely. Please upload it again." || message === "The room photo could not be made available for generation.")) {
    return "The reference image upload could not be completed.";
  }
  const safeMessages = new Set([
    "Only PNG and JPG images are accepted.",
    "The image must be 10MB or smaller.",
    "Upload could not be verified. Please upload the image again.",
    "The uploaded room photo is not valid for this session.",
    "The uploaded reference image is not valid for this session.",
    "The room photo could not be made available for generation.",
    "The reference image could not be made available for generation.",
  ]);
  if (safeMessages.has(message)) return message;
  return kind === "reference" ? "The reference image upload could not be completed." : "The room photo upload could not be completed.";
}
