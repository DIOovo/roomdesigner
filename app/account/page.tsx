...
import { PaymentStatus } from "@/components/account/payment-status";
import { PaymentTestCard } from "@/components/account/payment-test-card";
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ page?: string; checkout?: string }> }) {
  ...
  return (
    <main className="shell py-16 md:py-20">
      <div className="mx-auto max-w-4xl">
        {params.checkout === "success" ? <PaymentStatus initialCredits={entitlement.totalCreditsRemaining} /> : null}
        ...
        {canUsePaymentTest(user.email) ? <PaymentTestCard /> : null}
